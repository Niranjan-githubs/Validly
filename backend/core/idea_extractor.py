...
class IdeaExtractor:
    def __init__(self):
        self.client = Together()  # API key should be set as TOGETHER_API_KEY environment variable
...
When you think you have all the information, say: '✅ I'm ready to summarize your startup now.'"""
    ...
    def interactive_idea_extractor(self) -> Tuple[Optional[str], Optional[str]]:
        """Interactive idea extraction process"""
...
"""
        ...
        response = self.query_model(f"A user has shared their startup idea: '{user_idea}'. Please respond with your first question to understand their idea better.")
        print(f"\n🤖 {response}")
    ...
    def summarize_startup(self, user_idea: str, conversation: str) -> Dict[str, Any]:
...
"""
        ...
        response = self.query_model(summary_prompt)
...
if __name__ == "__main__":
    ...
    try:
        idea, conversation = extractor.interactive_idea_extractor()
        if idea and conversation:
            print("\n📦 Generating your startup summary...\n")
            summary = extractor.summarize_startup(idea, conversation)
