...
from agents.competitor_agent.venture_angel import main3
...
Also make sure to decide the domain based on their idea. 
...
async def competitor_agent(startup_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        from agents.competitor_agent.scraping_domain import main
        
        # Define functions to run in parallel
        def get_investors():
            return main3(prompt5)
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            # Submit both tasks
            futures = {
                executor.submit(get_investors): 'investors',
                executor.submit(get_competitor_data): 'competitor_data'
            }
            ...
            for future in as_completed(futures):
                ...
                try:
                    ...
                    if task_name == 'investors':
                        investors = result
                    elif task_name == 'competitor_data':
                        competitor_data = result
                    print(f"Completed: {task_name}")
                except Exception as exc:
                    print(f"Task {task_name} generated an exception: {exc}")
                    # Handle the exception based on which task failed
                    if task_name == 'investors':
                        investors = None
                    elif task_name == 'competitor_data':
                        competitor_data = None
        ...
        return {
            ...
            "analysis": f"Found {len(competitor_data)} competitors",
...
def get_domain_and_prompts(startup_data):
    ...
    prompt1 = f"top {domain_search} startups f6s india"
