...
class OptimizedFoundrScanPipeline:
    ...
    def __init__(self, firebase_service=None):
        ...
        self.idea_analyzer = StartupIdeaAnalyzer()
    ...
    def run_idea_analysis(self, startup_data: Optional[Dict] = None, uid: Optional[str] = None, session_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            idea, conversation = self.idea_analyzer.interactive_session()
            summary = self.idea_analyzer.generate_summary(idea, conversation)
    ...
    async def _run_competitor_analysis_async(self, startup_data: Dict[str, Any], uid: Optional[str] = None, session_id: Optional[str] = None) -> Tuple[Dict[str, Any], Dict]:
        """Run competitor analysis asynchronously and return both competitor and investor data"""
        try:
            ...
            datas = await competitor_agent(startup_data)

            investor_data = datas.pop("investor", None) if "investor" in datas else None

            # Now datas only contains competitor information
            competitor_data = datas  # This will have gaps, competitors, analysis, competitor_count, status
            
            formatted_investor_data = {
                "angel_investors": investor_data.get("angel_investors", []) if investor_data else [],
                "venture_capitalist": investor_data.get("venture_capitalist", []) if investor_data else []
            }
            ...
            if uid and session_id:
                ...
                if investor_data:
                    self.save_to_firestore(uid, session_id, "investor_landscape", formatted_investor_data)

            print(f"✅ Competitor analysis complete - found {competitor_data.get('competitor_count', 0)} competitors")
            return competitor_data, investor_data
    ...
    async def run_parallel_analysis(self, startup_data: Dict[str, Any], uid: Optional[str] = None, session_id: Optional[str] = None) -> Tuple[Dict[str, Any], Dict[str, Any], List[Any], Dict]:
        "Run all analyses in parallel with improved error handling - ALL THREE START SIMULTANEOUSLY"
        ...
        try:
            print("⏳ Creating all parallel tasks simultaneously...")
            
            # Get the event loop
            loop = asyncio.get_event_loop()
            
            # Create ALL THREE tasks at the same time - no sequential execution
            tasks = [
                # Task 1: Competitor analysis (async) - returns (competitor_data, investor_data)
                self._run_competitor_analysis_async(startup_data, uid, session_id),
                
                # Task 2: Market research (sync function in executor)
                loop.run_in_executor(
                    None, self._run_market_research_sync, startup_data, uid, session_id
                ),
                
                # Task 3: User pain analysis (sync function in executor)
                loop.run_in_executor(
                    None, self._run_user_pain_agent_sync, startup_data, uid, session_id
                )
            ]
            
            print("🚀 All three tasks started simultaneously!")
            print("⏳ Waiting for all analyses to complete...")
            
            # Wait for ALL tasks to complete at the same time
            results = await asyncio.gather(*tasks, return_exceptions=True)
            
            # Unpack results
            competitor_result, market_data, user_pain_points = results
            
            # Handle competitor result (returns tuple: competitor_data, investor_data)
            if isinstance(competitor_result, Exception):
                print(f"❌ Competitor analysis failed: {competitor_result}")
                competitor_data = {
                    "gaps": [],
                    "competitors": [],
                    "analysis": f"Analysis failed: {str(competitor_result)}",
                    "competitor_count": 0,
                    "status": "error"
                }
                investor_data = None
            else:
                # Unpack the tuple returned by _run_competitor_analysis_async
                competitor_data, investor_data = competitor_result
                print(f"✅ Competitor analysis complete - found {competitor_data.get('competitor_count', 0)} competitors")
                if investor_data:
                    print(f"✅ Investor data retrieved - {len(investor_data.get('angel_investors', []))} angels, {len(investor_data.get('venture_capitalist', []))} VCs")
            
            # Handle market research result
            ...
            # Handle user pain points result
            ...
            print("✅ All parallel analyses completed simultaneously!")
            return competitor_data, market_data, user_pain_points, investor_data
    ...
    def load_investor_data(self, investor_data: Dict = None) -> Tuple[List[Dict], List[Dict]]:
        """Load investor data from Firestore for the session if available, else use default data."""
        ...
        if investor_data and isinstance(investor_data, dict):
            if "venture_capitalist" in investor_data and investor_data["venture_capitalist"]:
                top_vcs = investor_data["venture_capitalist"]
            
            if "angel_investors" in investor_data and investor_data["angel_investors"]:
                top_angels = investor_data["angel_investors"]
    
        return top_vcs, top_angels

    def create_final_report(self, startup_data: Dict, competitor_data: Dict, market_data: Dict, user_pain_points: List = None, uid: Optional[str] = None, session_id: Optional[str] = None, investor_data: Dict = None) -> Dict:
        """Create comprehensive final report"""
        print("📋 Creating final report...")
        
        try:
            # Load investor data - now properly receives investor_data parameter
            top_vcs, top_angels = self.load_investor_data(investor_data)
    ...
    async def run_pipeline_async(self, startup_data: Optional[Dict] = None, uid: Optional[str] = None, session_id: Optional[str] = None) -> Dict:
        ...
        try:
            ...
            competitor_data, market_data, user_pain_points, investor_data = await self.run_parallel_analysis(startup_data, uid, session_id)
            ...
            final_report = self.create_final_report(
                ...
                uid,
                session_id,
                investor_data  # Now properly passed
            )
            ...
            try:
                ...
                risk_analysis = await risk_agent.analyze_startup_risks(
                    startup_data,
                    competitor_data,
                    market_data
                )
...
if __name__ == "__main__":
    ...
    # Example: Minimal sample startup data for testing
    sample_startup_data = {
        "title": "AI Diabetes Care Platform",
        "description": "A digital health platform using AI to help diabetes patients manage their health online.",
    ...
    # Optionally, get UID and session_id from command line or use defaults
    uid = sys.argv[1] if len(sys.argv) > 1 else "test_user"
    session_id = sys.argv[2] if len(sys.argv) > 2 else "test_session"

    print("🚀 Running FoundrScan Pipeline test...")
    pipeline = OptimizedFoundrScanPipeline()
    result = pipeline.run_pipeline(sample_startup_data, uid, session_id)
